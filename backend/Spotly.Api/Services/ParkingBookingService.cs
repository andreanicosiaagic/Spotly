...
public sealed class ParkingBookingService(
    ...
    OfficeTime officeTime)
{
    ...
    public async Task<IResult> GetMyBookingAsync(ClaimsPrincipal user, string? date)
    {
        ...
        return booking is null
            ? EndpointSupport.Problem(StatusCodes.Status404NotFound, "BOOKING_NOT_FOUND", "Nessuna prenotazione parcheggio attiva per la data selezionata.")
            : Results.Ok(booking);
    }

    public async Task<IResult> LockAsync(string spotId, ClaimsPrincipal user, string? date)
    {
        if (!EndpointSupport.TryDate(date, officeTime.Today, out var bookingDate)) return EndpointSupport.InvalidDate();
        var spot = await repo.GetSpotByIdAsync(spotId);
        if (spot is null) return EndpointSupport.Problem(StatusCodes.Status404NotFound, "RESOURCE_NOT_FOUND", "Risorsa non trovata.");
        if (!CurrentUser.CanBook(user, spot.Type))
            return EndpointSupport.Problem(StatusCodes.Status403Forbidden, "R08_INELIGIBLE", "R-08: utente non idoneo al posto speciale.");

        var acquired = await repo.TryAcquireLockAsync(spotId, CurrentUser.Id(user), bookingDate, TimeSpan.FromMinutes(BookingRules.LockDurationMinutes));
        return acquired
    ...
    public async Task<IResult> CreateBookingAsync(CreateParkingBookingRequest request, ClaimsPrincipal user)
    {
        ...
        await calendar.CreateEventAsync(CurrentUser.Id(user), "Prenotazione parcheggio", bookingDate, request.SpotId);
    ...
    public async Task<IResult> CancelAsync(string bookingId, ClaimsPrincipal user)
    {
        ...
        if (!outcome.Found) return EndpointSupport.Problem(StatusCodes.Status404NotFound, "BOOKING_NOT_FOUND", "Prenotazione parcheggio non trovata.");
