...
public sealed class InMemoryDeskRepository(IDbContextFactory<SpotlyDbContext> factory, TimeProvider clock) : IDeskRepository
{
    ...
    public Task<IEnumerable<DeskSpot>> GetSpotsAsync(string locationId, DateOnly date)
    {
        lock (Gate)
        {
            ...
            var bookings = db.DeskBookings.AsNoTracking()
                .Where(x => x.BookingDate == date && x.Status == BookingStatus.Active)
                .ToList();
            var result = db.DeskSpots.AsNoTracking()
                .Where(x => x.LocationId == locationId)
                .ToList()
                .Select(spot => spot with
                {
                    Status = bookings.Any(x => x.DeskId == spot.DeskId && x.LockedUntil == null)
                        ? ResourceStatus.Occupied
                        : bookings.Any(x => x.DeskId == spot.DeskId && x.LockedUntil > now)
    ...
    public Task<IEnumerable<DeskBooking>> GetBookingsByDateAsync(string locationId, DateOnly date)
    {
        ...
        return Task.FromResult<IEnumerable<DeskBooking>>(db.DeskBookings.AsNoTracking()
            .Where(x => x.BookingDate == date && ids.Contains(x.DeskId))
            .ToArray());
    }
    ...
    {
        using var db = factory.CreateDbContext();
        return Task.FromResult(db.DeskBookings.AsNoTracking().FirstOrDefault(x =>
            x.UserId == userId &&
            x.BookingDate == date &&
    ...
    public Task<bool> CheckInAsync(string bookingId, string userId, DateTime utcNow)
    {
        ...
        {
            using var db = factory.CreateDbContext();
            var booking = db.DeskBookings.FirstOrDefault(x =>
    ...
    public Task<bool> TryAcquireLockAsync(string deskId, string userId, DateOnly bookingDate, TimeSpan lockDuration)
    {
        lock (Gate)
        {
            using var db = factory.CreateDbContext();
            ...
            if (desk is null || desk.Status != ResourceStatus.Available) return Task.FromResult(false);
            if (db.DeskBookings.Any(x => x.UserId == userId && x.BookingDate == bookingDate && x.Status == BookingStatus.Active))
                return Task.FromResult(false);
            if (db.DeskBookings.Any(x => x.DeskId == deskId &&
                x.BookingDate == bookingDate &&
                x.Status == BookingStatus.Active &&
                (x.LockedUntil == null || x.LockedUntil > now)))
                return Task.FromResult(false);

            db.DeskBookings.Add(new DeskBooking
            {
                DeskId = deskId,
                UserId = userId,
